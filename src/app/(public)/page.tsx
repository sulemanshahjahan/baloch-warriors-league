            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {managers.map((m) => (
                <Link key={m.id} href={`/players/${m.slug}`} className="group">
                  <Card className="relative overflow-hidden h-full transition-all hover:-translate-y-1 hover:border-primary/50">
                    {/* Soft glow */}
                    <div className="absolute inset-x-0 -top-20 h-40 blur-3xl opacity-40 pointer-events-none bg-primary/15" />
                    <CardContent className="relative p-6 flex flex-col items-center text-center gap-4">
                      {/* Gradient-ringed avatar */}
                      <div className="rounded-full p-[3px] shrink-0 bg-gradient-to-br from-primary to-primary/40">
                        <SmartAvatar
                          type="player"
                          id={m.id}
                          name={m.name}
                      ...
                      <div>
                        <h3 className="font-black text-lg tracking-tight transition-colors group-hover:text-primary">
                          {m.name}
                        </h3>
                        <span className="mt-2 inline-flex items-center gap-1.5 text-xs font-bold tracking-widest uppercase px-3 py-1 rounded-full bg-primary/10 text-primary">
