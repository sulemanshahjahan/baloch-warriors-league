...
import { generateAndShareNewsCard } from "@/lib/share-news-card";
...
export function ShareNewsButton({ title, excerpt, slug }: ShareNewsButtonProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);

  const whatsappUrl = (() => {
    const shareUrl = `https://bwlleague.com/news/${slug}`;
    const text = [
      `📰 BWL News`,
      ``,
      `*${title}*`,
  ...
  const handleShare = async () => {
    ...
    if (!canvas) return;
    await generateAndShareNewsCard(canvas, { title, excerpt, slug });
  };
