...
export function ShareButtons({
  ...
  // Format match info (e.g., "Semi Final 1" or "Group A - Round 2")
  let matchInfo = "";
  if (round) {
    if (round.match(/Semi-final|Quarter-final|Final/i)) {
      // For knockout rounds, add match number
      const matchNum = matchNumber ? ` ${matchNumber}` : "";
      matchInfo = `${round}${matchNum}`;
    } else {
      // For group rounds, use as is
      matchInfo = round;
    }
  }
  ...
  const shareText = matchInfo 
    ? `${scoreText} | ${matchInfo} | ${tournamentName}`
    : `${scoreText} | ${tournamentName}`;
